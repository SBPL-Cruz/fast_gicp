...
FastGICPCuda<PointSource, PointTarget>::FastGICPCuda() {
  ...
  vgicp_cuda.reset(new FastGICPCudaCore());
...
template<typename PointSource, typename PointTarget>
void FastGICPCuda<PointSource, PointTarget>::setRotationEpsilon(double eps) {
  rotation_epsilon_ = eps;
}

template<typename PointSource, typename PointTarget>
void FastGICPCuda<PointSource, PointTarget>::setResolution(double resolution) {
...
template<typename PointSource, typename PointTarget>
void FastGICPCuda<PointSource, PointTarget>::setNearesetNeighborSearchMethod(NearestNeighborMethod method) {
...
std::vector<int> FastGICPCuda<PointSource, PointTarget>::find_neighbors_parallel_kdtree(int k, const boost::shared_ptr<const pcl::PointCloud<PointT>>& cloud, pcl::search::KdTree<PointT>& kdtree) const {
...
template<typename PointSource, typename PointTarget>
void FastGICPCuda<PointSource, PointTarget>::computeTransformation(PointCloudSource& output, const Matrix4& guess) {
